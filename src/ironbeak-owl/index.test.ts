...
describe('ironbeak-owl', () => {
    ...
    const cardC = handA.refer.order.find(item => item instanceof IronbeakOwlModel);
    const cardD = boardA.refer.order.find(item => item instanceof AncientWatcherModel);
    ...
    test('ironbeak-owl-play', async () => {
        ...
        expect(handA.refer.order.length).toBe(1); // Ironbeak Owl in hand
        expect(boardA.refer.order.length).toBe(1); // Ancient Watcher on board
        ...
        expect(boardA.refer.order.length).toBe(2); // Ironbeak Owl + Ancient Watcher on board
        expect(handA.refer.order.length).toBe(0); // Ironbeak Owl moved to board
