...
describe('razorfen-hunter', () => {
    ...
    const cardC = handA.refer.order.find(item => item instanceof RazorfenHunterModel);
    ...
    test('razorfen-hunter-play', async () => {
        ...
        expect(handA.refer.order.length).toBe(1); // Razorfen Hunter in hand
        ...
        expect(boardA.refer.order.length).toBe(2); // Razorfen Hunter + Boar on board
        expect(handA.refer.order.length).toBe(0); // Razorfen Hunter moved to board
        expect(playerA.child.mana.state.current).toBe(7); // 10 - 3 = 7

        // Check that a Boar was summoned
        const cardD = boardA.refer.order.find(item => item instanceof BoarModel);
