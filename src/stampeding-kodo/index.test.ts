...
describe('stampeding-kodo', () => {
    ...
    const cardC = handA.refer.order.find(item => item instanceof StampedingKodoModel);
    const cardD = boardB.refer.order.find(item => item instanceof WispModel);
    const cardE = boardB.refer.order.find(item => item instanceof ChillwindYetiModel);
    ...
    test('stampeding-kodo-play', async () => {
        ...
        expect(handA.refer.order.length).toBe(1); // Stampeding Kodo in hand
        expect(boardA.refer.order.length).toBe(0); // No minions on board
        expect(boardB.refer.order.length).toBe(2); // Wisp and Chillwind Yeti on board
        ...
        expect(boardA.refer.order.length).toBe(1); // Stampeding Kodo on board
        expect(handA.refer.order.length).toBe(0); // Stampeding Kodo moved to board
        expect(playerA.child.mana.state.current).toBe(5); // 10 - 5 = 5

        // Wisp should be destroyed (random target with 2 or less attack)
        expect(boardB.refer.order.length).toBe(1); // Only Chillwind Yeti remains
