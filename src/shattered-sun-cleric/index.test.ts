...
describe('shattered-sun-cleric', () => {
    ...
    const cardC = handA.refer.order.find(item => item instanceof ShatteredSunClericModel);
    const cardD = boardA.refer.order.find(item => item instanceof WispModel);
    const cardE = handB.refer.order.find(item => item instanceof ShatteredSunClericModel);
    const cardF = handB.refer.order.find(item => item instanceof WispModel);
    ...
    test('shattered-sun-cleric-battlecry', async () => {
        ...
        expect(boardA.refer.order.length).toBe(1); // Wisp on board
        expect(handA.refer.order.length).toBe(1); // Shattered Sun Cleric in hand
        ...
        expect(boardA.refer.order.length).toBe(2); // Wisp + Shattered Sun Cleric on board
        expect(handA.refer.order.length).toBe(0); // Shattered Sun Cleric moved to board
    ...
    test('shattered-sun-cleric-play', async () => {
        ...
        expect(handB.refer.order.length).toBe(2); // Shattered Sun Cleric + Wisp in hand
        ...
        // Shattered Sun Cleric should be on board
        expect(boardB.refer.order.length).toBe(1);
        expect(handB.refer.order.length).toBe(1); // Only Wisp left in hand
        ...
        expect(boardB.refer.order.length).toBe(2);
        expect(handB.refer.order.length).toBe(0);
    ...
    test('wisp-attack', async () => {
        ...
        expect(roleF.child.health.state.current).toBe(1);
        expect(boardA.refer.order.length).toBe(2); // Wisp + Shattered Sun Cleric on board
        expect(boardB.refer.order.length).toBe(2); // Wisp + Shattered Sun Cleric on board
        ...
        // Board state
        expect(boardA.refer.order.length).toBe(2); // Wisp + Shattered Sun Cleric on board
        expect(boardB.refer.order.length).toBe(1); // Only Shattered Sun Cleric on board
    });
});
