...
describe('injured-blademaster', () => {
    ...
    const cardC = handA.refer.order.find(item => item instanceof InjuredBlademasterModel);
    const cardD = handA.refer.order.find(item => item instanceof EarthenRingFarseerModel);
    ...
    test('injured-blademaster-play', async () => {
        ...
        expect(handA.refer.order.length).toBe(2); // Injured Blademaster in hand
        ...
        expect(boardA.refer.order.length).toBe(1); // Injured Blademaster on board
        expect(handA.refer.order.length).toBe(1); // Injured Blademaster moved to board
    ...
    test('earthen-ring-farseer-heal', async () => {
        // Check initial state
        expect(roleC.child.health.state.current).toBe(3); // Injured Blademaster: 4/3 (damaged)
        expect(handA.refer.order.length).toBe(1); // Earthen Ring Farseer in hand
        expect(boardA.refer.order.length).toBe(1); // Injured Blademaster on board
        ...
        expect(boardA.refer.order.length).toBe(2); // Injured Blademaster + Earthen Ring Farseer on board
        expect(handA.refer.order.length).toBe(0); // Earthen Ring Farseer moved to board
