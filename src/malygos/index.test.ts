...
describe('malygos', () => {
    ...
    const cardC = handA.refer.order.find(item => item instanceof ArcaneMissilesModel);
    const cardD = handA.refer.order.find(item => item instanceof FireballModel);
    const cardE = boardA.refer.order.find(item => item instanceof MalygosModel);
    ...
    test('arcane-missiles-cast', async () => {
        ...
        expect(boardA.refer.order.length).toBe(1); // Malygos on board
        expect(handA.refer.order.filter(item => item instanceof ArcaneMissilesModel).length).toBe(1);
        ...
        expect(handA.refer.order.filter(item => item instanceof ArcaneMissilesModel).length).toBe(0);
    });
