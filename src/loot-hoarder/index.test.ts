...
describe('loot-hoarder', () => {
    ...
    const cardC = handA.refer.order.find(item => item instanceof LootHoarderModel);
    const cardD = boardB.refer.order.find(item => item instanceof StranglethornTigerModel);
    const cardE = deckA.refer.order.find(item => item instanceof WispModel);
    ...
    test('loot-hoarder-play', async () => {
        ...
        expect(handA.refer.order.length).toBe(1); // Loot Hoarder in hand
        ...
        expect(boardA.refer.order.length).toBe(1); // Loot Hoarder on board
        expect(handA.refer.order.length).toBe(0); // Loot Hoarder moved to board
    ...
    test('loot-hoarder-death', async () => {
        ...
        expect(boardA.refer.order.length).toBe(1); // Loot Hoarder on board
        expect(boardB.refer.order.length).toBe(1); // Stranglethorn Tiger on board
        expect(handA.refer.order.length).toBe(0); // No cards in hand initially
        ...
        expect(boardA.refer.order.length).toBe(0); // Loot Hoarder dies
        expect(cardC.child.dispose.status).toBe(true);
        
        // Deathrattle should draw a card
        expect(handA.refer.order.length).toBe(1); // Drew a card from deathrattle
