...
describe('twilight-drake', () => {
    ...
    const cardC = handA.refer.order.find(item => item instanceof TwilightDrakeModel);
    ...
    test('twilight-drake-play', async () => {
        ...
        expect(handA.refer.order.length).toBe(3); // Twilight Drake + 2 Wisp in hand
        ...
        expect(boardA.refer.order.length).toBe(1); // Twilight Drake on board
        expect(handA.refer.order.length).toBe(2); // 2 Wisp remaining in hand
