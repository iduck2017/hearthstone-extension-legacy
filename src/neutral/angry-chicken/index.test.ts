...
import { GameModel, BoardModel, PlayerModel, MageModel, CommonUtil, ManaModel, HandModel, DeckModel } from "hearthstone-core";
...
describe('angry-chicken', () => {
    ...
    test('angry-chicken-attack', async () => {
        ...
        expect(cardC.child.dispose.state.isActived).toBe(false); // A's chicken survived due to shield
      
        expect(cardD.child.dispose.state.isActived).toBe(true); // B's chicken died
    })
})
