...
describe('kobold-geomancer', () => {
    ...
    const cardC = handA.refer.order.find(item => item instanceof KoboldGeomancerModel);
    ...
    test('kobold-geomancer-play', async () => {
        ...
        expect(handA.refer.order.length).toBe(2); // Kobold Geomancer + Fireball in hand
        ...
        expect(boardA.refer.order.length).toBe(1); // Kobold Geomancer on board
        expect(handA.refer.order.length).toBe(1); // Kobold Geomancer moved to board
