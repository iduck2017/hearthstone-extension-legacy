...
describe('demolisher', () => {
    ...
    const cardC = boardB.refer.order.find(item => item instanceof DemolisherModel);
    ...
    test('demolisher-hook', async () => {
        // Check initial state
        expect(boardA.refer.order.length).toBe(0); // Player A has empty board
        expect(boardB.refer.order.length).toBe(1); // Player B has Demolisher on board
