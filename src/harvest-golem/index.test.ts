...
describe('harvest-golem', () => {
    ...
    const cardC = handA.refer.order.find(item => item instanceof HarvestGolemModel);
    ...
    test('harvest-golem-play', async () => {
        ...
        expect(handA.refer.order.length).toBe(1); // Harvest Golem in hand
        ...
        expect(boardA.refer.order.length).toBe(1); // Harvest Golem on board
        expect(handA.refer.order.length).toBe(0); // Harvest Golem moved to board
    ...
    test('harvest-golem-death', async () => {
        ...
        expect(boardA.refer.order.length).toBe(1); // Damaged Golem summoned
        expect(cardC.child.dispose.status).toBe(true); // Harvest Golem dies
        
        // Check that Damaged Golem was summoned
        const cardE = boardA.refer.order.find(item => item instanceof DamagedGolemModel);
