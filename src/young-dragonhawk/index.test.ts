...
describe('young-dragonhawk', () => {
    ...
    const cardC = boardA.refer.order.find(item => item instanceof YoungDragonhawkModel);
    const cardD = boardB.refer.order.find(item => item instanceof ShieldbearerModel);
    ...
    test('young-dragonhawk-windfury', async () => {
        // Check initial state
        expect(boardA.refer.order.length).toBe(1); // Young Dragonhawk on board
        expect(boardB.refer.order.length).toBe(1); // Shieldbearer on board
